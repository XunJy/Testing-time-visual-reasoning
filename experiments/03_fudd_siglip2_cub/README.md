# Experiment 03: FuDD + SigLIP 2 on CUB-200-2011

Status: **planned; not yet implemented or run**.

This combination will reuse the validated FuDD method implementation and add a
separate SigLIP 2 model backend. It is scientifically distinct from both
FuDD + OpenAI CLIP and FuDD + SigLIP, so it owns independent configuration,
caches, predictions, and immutable run directories.

Before evaluation, lock the exact SigLIP 2 checkpoint and variant, resolution,
